...
- Развернуть кластер Apache Kafka. Написать ETL, который должен получать данные от источников и сохранять предобработанные данные в топики с учетом требований по семантикам, надежности и быстродействии;
...
Для работы проекта необходимо установить Flink на локальную машину и настроить конфигурацию кластера Flink согласно официальной документации

1. Для запуска redis перейдите в папку ```ProducerPython/pythonKafka/redis-docker``` и выполните команду ```docker-compose build``` и ```docker-compose up -d```
   
2. Для запуска kafka перейдите в папку ```ProducerPython/pythonKafka/kafka-docker``` и выполните команду ```docker-compose build``` и ```docker-compose up -d```
   
3. Запустите класс ```kafkaProducer.py``` для получения данных с Binance в реальном времени
   
4. В главном модуле выполните в Gradle shadowJar. В папке модуля build/libs сформируется JAR файл
   
5. Далее, находясь в папке с FLink (под этим понимается, что FLink установлен, а кластер настроен) выполните команду: ```./bin/start-cluster.sh```. 
   В кластере сделайте ```submit job``` и поместите туда сформированный (на шаге 4) Jar файл. 
   Теперь Flink обрабатывает данные, поступающие ему из топика Kafka в реальном времени
   
6. Перейдите в модуль ```ClientServerApp``` и запустите класс ```MainApplication```. Затем перейдите по адресу ```http://localhost:8084/chart```. 
   Вы увидите график изменения агрегированных котировок криптовалюты (агрегация под будущее скользящее среднее) в реальном времени. 
   Со временем поступления новых данных график будет обновляться и сдвигаться. 
   Конфигурацию ```Spring Server``` можно измененить в модуле ```ClientServerApp``` в папке ```resources/application.properties```

```
Для остановки кластера выполните команду: ./bin/stop-cluster.sh

(Не останавливайте кластер до пункта 6, поскольку иначе вы не увидете формирующийся график в фронт части приложения, поскольку WebSocketServer не будет реагировать на новые данные в Kafka, поступающие после обработки Flink)
```
...
![my-amazing-crypto-graph.png](assets%2Fmy-amazing-crypto-graph.png)

Пример работы Flink:

![my-amazing-flink.png](assets%2Fmy-amazing-flink.png)

## Итог
В рамках дисциплины **"Потоковый анализ данных"** реализован учебный прототип системы крипто-аналитики **"Streaming Crypto Analysis"**

### Реализовано
- Найден и использован источник потоковых данных Binance API;
- Развернут кластер Apache Kafka, написан ETL, получающий данные из Binance API и сохраняющий предобработанные данные в топики с учетом требований по семантикам, надежности и быстродействию;
- Развернут Apache Flink, настроено чтение из топиков Kafka. Реализованы необходимые конвейеры для обработки потоков данных;
- Созданы потоковое API и потоковый клиент. В качестве пользовательского интерфейса было разработано веб-приложение.
